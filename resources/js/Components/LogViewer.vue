...
<template>
    <div class="flex h-full flex-col">
        ...
        <div class="flex flex-wrap items-center gap-2 border-b border-neutral-200 px-3 py-2 dark:border-neutral-800">
            ...
            <span class="text-[10px] text-neutral-400">{{ filtered.length }} entries</span>
        ...
        <div ref="containerEl" class="min-h-0 flex-1 overflow-auto bg-neutral-50 p-2 font-mono text-xs dark:bg-neutral-950">
            <div v-if="!filtered.length" class="p-4 text-center text-neutral-400">
            ...
            >
                <div
                    ...
                    @click="entry.details.length && toggleEntry(index)"
                >
                    <span class="mt-1 h-2 w-2 shrink-0 rounded-full" :class="levelStyle(entry.level).dot"></span>
                    <span v-if="entry.timestamp" class="shrink-0 text-neutral-400">{{ entry.timestamp }}</span>
                    <span class="shrink-0 font-semibold" :class="levelStyle(entry.level).text">
                        {{ (entry.originalLevel || entry.level).toUpperCase() }}
                    </span>
                    <span class="break-words text-neutral-800 dark:text-neutral-200">{{ entry.message }}</span>
                    <span v-if="entry.details.length" class="ml-auto shrink-0 text-neutral-400">
                        {{ expanded.has(index) ? '▾' : '▸' }}
                    </span>
                </div>

                <pre
                    v-if="entry.details.length && expanded.has(index)"
                    class="mt-1 whitespace-pre-wrap break-words pl-4 text-neutral-500"
                >{{ entry.details.join('\n') }}</pre>
            </div>
        </div>
    </div>
